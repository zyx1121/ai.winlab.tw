...
export default function RecruitmentEditPage() {
  ...
  useEffect(() => {
    ...
    if (user) fetchRecruitment();
  }, [user, authLoading, fetchRecruitment, router]);

  useEffect(() => {
    if (!authLoading && user && !isAdmin) router.push("/recruitment");
    ...
    <div className="max-w-6xl mx-auto px-4 flex flex-col mt-8 pb-16">
      <div className="sticky top-16 z-20 bg-background/80 backdrop-blur-sm py-4 -mx-4 px-4 flex flex-col gap-4">
        <div className="flex items-center justify-between gap-4">
          <Button variant="ghost" size="sm" onClick={() => guardNavigation(() => router.push("/recruitment"))}>
          ...
          <div className="flex flex-col gap-2">
            {positions.map((pos, idx) => (
              <div key={idx} className="flex items-center gap-2">
                ...
                <Input
                  value={pos.location ?? ""}
                  onChange={(e) => updatePosition(idx, "location", e.target.value || null)}
                  placeholder="地區"
                  className="w-32"
                />
                <div className="flex items-center gap-1.5 shrink-0">
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      updatePosition(idx, "count", isNaN(v) || v < 1 ? 1 : v);
                    }}
